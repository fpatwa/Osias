...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Dictionary of values containing the following: Number_of_VM_Servers, vCPU, RAM_in_MB, HDD1, HDD2, Internal_CIDR, Public_CIDR, Data_CIDR, VM_Deployment_CIDR" +
                ...
                "VM_Deployment_CIDR is a small /28 CIDR used for assigning a public IP to the VM, allocating IP's as floating IPs for OpenStack, and a VIP address for horizon" + 
                "{'vCPU': 8, 'RAM_in_MB': 16384, 'HDD1': 60, 'HDD2': 10, 'HDD3': 10, 'HDD4': 10, 'Internal_CIDR': '192.168.1.0/24', 'Number_of_VM_Servers': 3, 'Public_CIDR': '10.245.121.0/24', 'Data_CIDR': '10.100.0.0/16', 'DNS_IP': '10.250.53.202'}")
    parser.add_argument(
        "operation",
        type=str,
        choices=['cleanup',
...
def create_virtual_servers(maas_url, maas_api_key, vm_profile):
    ...
    public_ips = {}
    num_Servers = sorted([3, int(vm_profile['Number_of_VM_Servers']), 7])[1] # Keeps the limit of VM's created from 3-7 VM's.
    ...
    internal_ips = servers.get_machines_interface_ip(server_list, machines_info, "eno1", "internal")
    data_ips = servers.get_machines_interface_ip(server_list, machines_info, "eno3", "data")
...
def post_deploy_openstack(servers_public_ip, pool_start_ip, pool_end_ip, dns_ip):
    if pool_start_ip:
        utils.run_script_on_server('post_deploy_openstack.sh', servers_public_ip[0], args=[dns_ip, pool_start_ip, pool_end_ip])
    elif dns_ip:
        utils.run_script_on_server('post_deploy_openstack.sh', servers_public_ip[0], args=[dns_ip])
    else:
        ...
        if args.operation == 'cleanup':
           cleanup(servers_public_ip, storage_nodes_public_ip)
            ...
            bootstrap_openstack(servers_public_ip, controller_nodes, network_nodes,
                                storage_nodes_private_ip, compute_nodes, monitoring_nodes, raid,
                                docker_registry, docker_registry_username, args.DOCKER_REGISTRY_PASSWORD, VM_CIDR)
            ...
            if args.MAAS_URL and args.MAAS_API_KEY:
                delete_virtual_machines(servers_public_ip, args.MAAS_URL, args.MAAS_API_KEY)
            else:
                raise Exception(
                    'ERROR: MAAS_API_KEY and/or MAAS_URL argument not specified.\n' +
